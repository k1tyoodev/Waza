...
`**xxx**。content` 这种 bold 起手段落本身没问题，但 4 个以上连续出现会变成 AI 模板感。两种处理：

**4+ 项连续**：把 `**xxx**。` 改成 `**xxx**，`，让 bold 变成句子的承重词，而不是独立小标题。整段读起来是 prose，不是工整 bold 列表。

**2-3 项零散分布**：保留 `**xxx**。` 没问题，节奏不密集。

判断信号：往前数 3 个段落，如果都是 `**xxx**。` 起手，第 4 段必须打破节奏。
